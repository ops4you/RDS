...
gport_table = { "10.0.1.254" : 1, "10.0.2.254" : 2 , "10.0.3.254" : 3}
...
class Switchl3(app_manager.RyuApp):
    ...
    def arp_reply(self, datapath, eth_p, arp_p, in_port):
        
        gp = gport_table.get(arp_p.dst_ip)
        mg = self.router_ports.get(gp)
    ...
    def arp_request(self,datapath,ipv4_p):
        port = hport_table.get(ipv4_p.dst)
        ip_source = self.get_key_from_value(gport_table,port)
        mac_source = self.router_ports.get(port)
    ...
    def icmp_reply(self, datapath, eth_p, icmp_p, ipv4_p, in_port):
        
        if icmp_p.type != icmp.ICMP_ECHO_REQUEST:
            return

        gp = gport_table.get(ipv4_p.dst)
        mg = self.router_ports.get(gp)
    ...
    def icmp_unreachable(self, datapath, eth_p, icmp_p, ipv4_p, in_port):
        ...
        mg = self.router_ports.get(gateway)
    ...
    def port_desc_stats_reply_handler(self, ev):
       for p in ev.msg.body:
            self.router_ports.update({p.port_no : p.hw_addr})
    ...
    def _packet_in_handler(self, ev):
        ...
        if arp_p:
            if arp_p.dst_ip in gport_table.keys() and arp_p.opcode == arp.ARP_REQUEST:
                self.ip_to_mac.update({arp_p.src_ip : arp_p.src_mac})
                self.logger.info("Received ARP Packet %s %s %s ", dpid, src, dst)
                self.arp_reply(datapath, eth_p, arp_p, in_port)
            elif arp_p.dst_ip in gport_table.keys() and arp_p.opcode == arp.ARP_REPLY:
                self.ip_to_mac.update({arp_p.src_ip : arp_p.src_mac})
                for m in self.message_queue[arp_p.src_ip]:      
                    p = packet.Packet(m.data)
                    p_eth = pkt.get_protocol(ethernet.ethernet)
                    out_port = hport_table.get(arp_p.src_ip)
                    p_eth.src = self.router_ports.get(out_port)
                    p_eth.dst = self.ip_to_mac.get(arp_p.src_ip)
                    parser = datapath.ofproto_parser  
                    ofproto = datapath.ofproto
                    p.serialize()
        ...
        if ipv4_p:
         if ipv4_p.dst in gport_table.keys():
              icmp_p = pkt.get_protocol(icmp.icmp)
              if icmp_p:
                self.icmp_reply(datapath, eth_p, icmp_p, ipv4_p, in_port) 
                return
         else:
            if ipv4_p.dst in hport_table.keys():
              if ipv4_p.dst in self.ip_to_mac.keys():
                port = hport_table.get(ipv4_p.dst)
                eth_p.src = self.router_ports.get(port)
                eth_p.dst = self.ip_to_mac.get(ipv4_p.dst)
                parser = datapath.ofproto_parser  
                ofproto = datapath.ofproto
                pkt.serialize()
                actions = [datapath.ofproto_parser.OFPActionOutput(port)]
                ...
                self.arp_request(datapath, ipv4_p)
                return
